...
try {
    ...
    Write-Host "`n🧙‍♂️ ENHANCED DZIP ANALYSIS WITH GAME INTELLIGENCE 🧙‍♂️" -ForegroundColor Green
    ...
    if ($analysis.CategorizedFindings.Count -gt 0) {
        Write-Host "`n📊 Pattern Categories Found:" -ForegroundColor Yellow
    ...
    if ($analysis.GameIntelligence.Count -gt 0) {
        Write-Host "`n🎯 GAME INTELLIGENCE INSIGHTS:" -ForegroundColor Green
    ...
    if ($analysis.CriticalDecisions.Count -gt 0) {
        Write-Host "`n⚔️ CRITICAL PLAYER DECISIONS DETECTED:" -ForegroundColor Red
        foreach ($decision in $analysis.CriticalDecisions) {
            if ($decision.Found) {
                $ctx = $decision.Context
                Write-Host "    🎭 $($ctx.decision_name)" -ForegroundColor Yellow
    ...
    if ($analysis.QuestData.Count -gt 0) {
        Write-Host "`n📜 QUEST PROGRESSION DATA:" -ForegroundColor Blue
        foreach ($quest in $analysis.QuestData) {
            Write-Host "    📖 $($quest.quest_name)" -ForegroundColor Cyan
    ...
    Write-Host "`n📝 Raw Text Preview:" -ForegroundColor Gray
